...
OUTER_URL = "https://cyberbot.in.tum.de/invite"

DB_PATH = "./invitations.sqlite"
...
class InviteListener:
    ...
    async def start(self, nio_client):
        """
        start servers
        """
        if self.is_running:
...
<body>
    <p>{msg}</p>
            ...
            with sqlite3.connect(self.dbname) as conn:
                conn.execute('''CREATE TABLE IF NOT EXISTS invitations (random_part text, room_id text, invitor text)''')
                conn.commit()
                cur = conn.cursor()
                cur.execute('''SELECT room_id, invitor FROM invitations WHERE random_part = ?''', (random_part,))
                res = cur.fetchall()
            ...
            if request.method == "GET":
                ...
                content = gen_site_content(room_name,res[0][1])
    ...
    async def register_invite(self, room, random_part, invitor):
        with sqlite3.connect(self.dbname) as conn:
            conn.execute('''CREATE TABLE IF NOT EXISTS invitations (random_part text, room_id text, invitor text)''')
            conn.execute('''INSERT INTO invitations VALUES (?,?,?)''', (random_part, room.room_id, invitor))
            conn.commit()
...
async def register_to(plugin):
    ...
    async def invite_callback(room, event):
        ...
        await il.register_invite(room, random_part, invitor)
